...
from config import OCR_LANGUAGE, OCR_USE_ANGLE_CLS
...
class PageData:
    ...
    method: str = "native"           # "native" | "ocr" | "docx"
...
def _parse_docx(path: Path) -> List[PageData]:
    ...
    # DOCX has no concept of pages — treat whole doc as page 1
    return [PageData(
        source=path.name, page_num=1,
        text="\n".join(paras), tables=[], method="docx"
    )]
...
def load_document(path: str | Path) -> List[PageData]:
    ...
    logger.info(f"Loading document: {path.name}  (type={suffix})")
    ...
    if suffix == ".pdf":
        ...
        raise ValueError(f"Unsupported file type: {suffix}  "
                         f"(supported: pdf, png, jpg, jpeg, tiff, docx)")
